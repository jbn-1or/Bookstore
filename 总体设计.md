...
本系统是一个基于命令行交互的书店管理系统，面向顾客、销售人员和店长三类角色，通过权限分级实现差异化功能访问。系统采用C++编写，所有主体数据实时存储于外存文件，支持嵌套登录（多账户同时登录，指令默认由最后登录账户执行）。核心功能包括：
...
系统首次运行时自动完成初始化，创建超级管理员账户（root，密码sjtu，权限{7}），通过命令行指令接收操作，直至读入EOF或`quit`/`exit`指令终止。
...
2. **命令处理阶段**：
   - 读取用户输入指令，去除冗余空格后进行语法解析
   - 验证指令格式合法性（关键词、参数个数及格式）
...
│  ├─ Lexer.h             // 负责将输入的每行分解为一系列的标记（tokens），以便后续解析
│  ├─ Parser.h            // Parser类声明（命令解析）
│  ├─ Storage.h           // 数据存储接口声明
│  └─ Utils.h             // 工具函数声明（时间、文件IO、权限校验、文件读写）
└─ src/
   ├─ User.cpp
   ├─ Book.cpp
   ├─ Transaction.cpp
...
1. **用户管理模块**
   ...
   - 权限控制：严格遵循{0}⊂{1}⊂{3}⊂{7}的子集关系，低权限功能包含于高权限
...
3. **交易管理模块**
   ...
   - 关联关系：交易记录与操作人、图书ISBN强关联，支持时间范围筛选

4. **日志管理模块**
   - 核心功能：操作日志记录（所有操作自动生成）、财务日志查询（{7}专属）、员工工作报告（{7}专属，按用户筛选）、系统日志导出（{7}专属）
   - 日志内容：包含时间戳、操作人、操作类型及详情，支持按时间范围检索
...
- 总文件数≤20（当前设计4个核心文件，预留扩展空间）
- 所有数据实时写入外存，操作过程中不缓存全量数据
- 采用制表符（\t）分隔字段，换行符（\n）分隔记录，支持逐行读写
- 数值型数据（价格、数量等）严格校验格式，确保解析一致性
...
2. **Book类**
   ...
   public:
       ...
       // 库存操作
       bool increase_quantity(int num);  // 增加库存（进货）
       bool decrease_quantity(int num);  // 减少库存（购买）
       // 修改属性（返回是否成功）
       bool set_name(const std::string& new_name);
       bool set_author(const std::string& new_author);
       bool set_keyword(const std::string& new_keyword);  // 校验无重复
       bool set_price(double new_price);
       bool set_isbn(const std::string& new_isbn);        // 校验非原ISBN
       // 匹配查询条件
...
5. **管理器类（示例：UserManager）**
   ...
   public:
       ...
       bool login(const std::string& user_id, const std::string& pwd);  // 登录并压栈
       bool logout();                         // 登出并弹栈
       bool create_user(const std::string& user_id, const std::string& pwd, int priv, const std::string& name, int current_priv);  // 权限检查
