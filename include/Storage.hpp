...
const int MAX_ENTRIES_PER_BLOCK = 1000;
...
// struct Object {
//     char index[MAX_INDEX_LEN];  // 索引字符串（固定长度）
//     int value;                  // 数值

//     // 从string初始化index
//     void set_index(const string& idx) {
//         std::strncpy(index, idx.c_str(), MAX_INDEX_LEN - 1);
...
//     // 实例化Tem=Object，文件名=Out.data，索引长度=MAX_INDEX_LEN，块最大条目=MAX_ENTRIES_PER_BLOCK
//     BlockListManager<Object> manager("Out.data", MAX_INDEX_LEN, MAX_ENTRIES_PER_BLOCK);
...
//             tmp.set_index(index);
//             tmp.value = value;
//             manager.deleteEntry(tmp);
//         } else if (cmd == "find") {
//             string index;
//             std::cin >> index;
//             std::vector<Object> arr;
//             arr.clear();
//             if (manager.findEntry(index, arr)) {
//                 for (int i = 0; i*1ULL < arr.size(); ++i) {
//                     std::cout << arr[i] << " ";
//                 }
//                 std::cout << "\n";
//             } else {
